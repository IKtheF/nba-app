    ...
    if st.button('採点'):
        ...
        if cnt!=100:
            ...
            if ans3!='トロント':
                ...
                st.write('答え：トロント')
            if ans4!='ステフィンカリー':
                st.write('1シーズンでの3ポイント成功数歴代1位の402本を記録した選手は誰？')
                st.write('答え：ステフィンカリー')
            if ans5!='ルカドンチッチ':
                st.write('スロベニア出身で2019年の新人王に選ばれた、「欧州の神童」と言えば誰？')
                st.write('答え：ルカドンチッチ')
            if ans6!='ネッツ':
                st.write('ニューヨークに本拠地を置くNBAのチームは「ニューヨークニックス」と何？')
                st.write('答え：ネッツ')
            if ans7!='ロサンゼルスレイカーズ':
                st.write('コービーブライアントがルーキー時代から引退するまで所属していたチームは？')
                st.write('答え：ロサンゼルスレイカーズ')
            if ans8!='ジェームスハーデン':
                st.write('"Fear the Beard"で知られる左利きの選手は誰？')
                st.write('答え：ジェームスハーデン')
            if ans9!='ヤニスアデトクンポ':
                st.write('"The Greek Freak"で知られるギリシャ出身の選手は誰？')
                st.write('答え：ヤニスアデトクンポ')
            if ans10!='ケビンデュラント':
                st.write('過去四度得点王に輝き、現在はブルックリンネッツに所属している選手は誰？')
                st.write('答え：ケビンデュラント')
            if ans11!='ロサンゼルスレイカーズ':
                st.write('2000年以降（1999-2000シーズンも含んで数えても良い）３連覇を達成した球団はどこ？')
                st.write('答え：ロサンゼルスレイカーズ')
            if ans12!='1':
                st.write('コービーブライアントは現役時代、何度シーズンMVPを獲得したでしょう')
                st.write('答え：1')
            if ans13!='サンアントニオスパーズ':
                st.write('2010年代のマイアミヒートの3連覇を阻んだチームはどこでしょう')
                st.write('答え：サンアントニオスパーズ')
            if ans14!='ブレイクグリフィン':
                st.write('2009年ドラフトで1巡目1位指名を受けたのは誰？')
                st.write('答え：ブレイクグリフィン')
            if ans15!='2013-14':
                st.write('カワイレナードが初めてFMVPを獲得したシーズンは？')
                st.write('答え：2013-14')
            if ans16!='ダラスマーベリックス':
                st.write('マイアミヒートが初めて優勝した時のファイナルの相手は？')
                st.write('答え：ダラスマーベリックス')
            if ans17!='ドウェインウェイド':
                st.write('2003年でドラフト指名されたレブロン、ウェイド、ボッシュ、カーメロの中で最初にNBA制覇を達成したのは誰？')
                st.write('答え：ドウェインウェイド')
            if ans18!='トニーパーカー':
                st.write('2006-07ファイナルでFMVPを獲得したのは誰？')
                st.write('答え：トニーパーカー')
            if ans19!='24':
                st.write('ポールジョージがNBAに入ってから数年つけていた13の前の背番号は？')
                st.write('答え：24')
            if ans20!='カーメロアンソニー':
                st.write('このロゴは誰のものでしょう？')
                st.write('答え：カーメロアンソニー')
            if ans21!='クレイトンプソン':
                st.write('このロゴは誰のものでしょう？')
                st.write('答え：クレイトンプソン')
            if ans22!='マーケット大学':
                st.write('ジミーバトラー、ウェスリーマシューズ、ジェイクラウダーの出身大学は？')
                st.write('答え：マーケット大学')
            if ans23!='デンバーナゲッツ':
                st.write('どのチームのマスコットでしょう？')
                st.write('答え：デンバーナゲッツ')
            if ans24!='トレバアリーザ':
                st.write('NBA史上で最もトレードされた回数が多い選手は？')
                st.write('答え：トレバアリーザ')
            if ans25!='ジェリーウェスト':
                st.write('NBAのロゴになっている選手は誰？')
                st.write('答え：ジェリーウェスト')
            if ans26!='6.5号':
                st.write('NBAが使用しているバスケットボールの大きさは？')
                st.write('答え：6.5号')
            if ans27!='サンアントニオスパーズ':
                st.write('2010/01/01 ~ 2019/12/31の期間で最も勝利数が多いのはどのチームでしょう（シーズンのみ）')
                st.write('答え：サンアントニオスパーズ')
            if ans28!='オクラホマシティサンダー':
                st.write('2010/01/01 ~ 2019/12/31の期間で2番目に勝利数が多いのはどのチームでしょう（シーズンのみ）')
                st.write('答え：オクラホマシティサンダー')
            if ans29!='サージイバカ':
                st.write('2010年代の勝利数が多い1位はレブロンジェームス、3位はジェームスハーデン、では2位は？')
                st.write('答え：サージイバカ')
            if ans30!='サクラメントキングス':
                st.write('2010/01/01 ~ 2019/12/31の期間で最も勝利数が少ないのはどのチームでしょう')
                st.write('答え：サクラメントキングス')
            if ans31!='ミネソタティンバーウルブズ':
                st.write('球団設立から現在までで、最も勝率が低いのはどのチームでしょう（2021/08/18時点で）')
                st.write('答え：ミネソタティンバーウルブズ（39%）')
            if ans32!='ゴールデンステイトウォリアーズ':
                st.write('2021/08/18の時点でTwitterのフォロワー数が最も多いのはレイカーズ、では第2位は？')
                st.write('答え：ゴールデンステイトウォリアーズ')
            if ans33!='マイアミヒート':
                st.write('球団創設以来、一度もドラフト1巡目1位指名をしていないものの優勝を果たしたのはどのチームでしょう')
                st.write('答え：マイアミヒート')
            if ans34!='スティーブナッシュ':
                st.write('Sportsnetの解説者によってよく使われる"Captain Canada"とは誰のニックネームでしょう')
                st.write('答え：スティーブナッシュ')
            if ans35!='メッタワールドピース':
                st.write('中国リーグへの移籍を機に"ザ・パンダズ・フレンド"に改名した選手は？')
                st.write('答え：メッタワールドピース')
            if ans36!='33':
                st.write('1971年にロサンゼルスレイカーズが記録した、現在でも上回られていない最長連勝記録は何連勝でしょう')
                st.write('答え：33')
            if ans37!='ラシードウォーレス':
                st.write('1シーズンでテクニカルファールを最も取られたのは誰？')
                st.write('答え：ラシードウォーレス')
            if ans38!='レブロンジェームス':
                st.write('2021/09/21の時点で現役の選手の中で、最も通算出場時間が長いのは誰？')
                st.write('答え：レブロンジェームス')
            if ans39!='ジェイソンウィリアムズ':
                st.write('White Chocolateの愛称で親しまれた選手は？')
                st.write('答え：ジェイソンウィリアムズ')
            if ans40!='アンソニーデイビス':
                st.write('誰のタトゥーでしょう')
                st.write('答え：アンソニーデイビス')
            if ans41!='メンフィスグリズリーズ':
                st.write('ウォリアーズが73-9でシーズンを終えた2015-16シーズン、73勝目をかけた最終戦の相手は？')
                st.write('答え：メンフィスグリズリーズ')
            if ans42!='親戚':
                st.write('ビンスカーターとトレイシーマグレディの関係は？')
                st.write('答え：親戚')
            if ans43!='マジックジョンソン':
                st.write('NBAの長い歴史で唯一、引退後にファン投票でオールスターゲームに出場した選手は誰？')
                st.write('答え：マジックジョンソン')
            if ans44!='マイアミヒート':
                st.write('在籍したことがないにも関わらず、マイケルジョーダンの背番号を永久欠番にしている球団は？')
                st.write('答え：マイアミヒート')
            if ans45!='パットライリー':
                st.write('"Three-peat"（3連覇を意味する）という言葉の生みの親は誰でしょう？')
                st.write('答え：パットライリー')
            if ans46!='デリックローズ':
                st.write('NBA 2K13のパッケージを飾ったのは、ブレイクグリフィン、ケビンデュラント、あと一人は誰でしょう？')
                st.write('答え：デリックローズ')
            if ans47!='パッケージの選手が移籍':
                st.write('NBA 2K14 ~ 2K18の4年（2K16は除く）（20周年版のも含めると2K19までの5年間）続いたジンクスとは？')
                st.write('答え：パッケージの選手が移籍')
            if ans48!='クリスポール':
                st.write('NBA 2Kにおいて99 OVRを獲得した選手は歴代で6人いますが、現役ではレブロンとあと一人います。誰でしょう？')
                st.write('答え：クリスポール')
            if ans49!='75':
                st.write('NBAが創設されてから2021-22シーズンで何年目になる？')
                st.write('答え：75')
            if ans50!='大好きです最高！':
                st.write('NBAは好きですか？')
                st.write('答え：大好きです最高！')